 ...
 * @author Chirag
 */
public class Integration extends Applet {

    Point loc = new Point();
    double maxpos, maxneg, llmt, ulmt, Fx, x, nodx, nody, dx, min, Xp_d, Yp_d, Xp_dx, Yp_dy, Xsegwidth, Ysegwidth, NoXseg, NoYseg,ch=0.5,a1,b1,c1;
   ...
   addKeyListener(new KeyAdapter()
    ...
    {
        int i,Xp,Yp,Xu,Yu,j;
        double yu,xu=x_in[0],c,mu,pixel_dx,dm,stepx,xp,yp,diff;
        pixel_dx=(hpix)/(interval[TD-1][1]-interval[0][0]);
        Xp=0;
        Yp=0;
        ...
        for(i=0;i<num_input-1;i++)
        {
            ...
            yu=y_in[i];
            solveCoeff(dm,i);
            System.out.println("a1= "+a1+"b1= "+b1+"c1= "+c1+"xu="+xu);
    ...
    public void solveCoeff(double dm,int interval)
    {
        int i,j,n=3,k;
        double eqn[][] =new double[3][4];
        double ans[]=new double[3];
        eqn[2][0]=Math.pow(x_in[interval],2);
        eqn[2][1]=x_in[interval];
        eqn[2][2]=1;
        eqn[2][3]=y_in[interval];
        eqn[1][0]=Math.pow(x_in[interval+1],2);
        eqn[1][1]=x_in[interval+1];
        eqn[1][2]=1;
        eqn[1][3]=y_in[interval+1];
        eqn[0][0]=2*x_in[interval+1];
        eqn[0][1]=1;
        eqn[0][2]=0;
        eqn[0][3]=dm;
        ...
        if(x_in[interval+1]==0)
        {
        eqn[0][0]=Math.pow(x_in[interval],2);
        eqn[0][1]=x_in[interval];
        eqn[0][2]=1;
        eqn[0][3]=y_in[interval];
        eqn[2][0]=Math.pow(x_in[interval+1],2);
        eqn[2][1]=x_in[interval+1];
        eqn[2][2]=1;
        eqn[2][3]=y_in[interval+1];
        eqn[1][0]=2*x_in[interval+1];
        eqn[1][1]=1;
        eqn[1][2]=0;
        eqn[1][3]=dm;
        }
   ...
   public double Interpolation(double xc)
    {
        int i,d;
        double yc,mu,xp,yp,dm,stepx,diff,min;
   ...
   {
        int i,j;
        double sum=0,lsum=0,dm;
        for(i=0;i<num_input;i++)
        {
            dm=m[i]+(m[i+1]-m[i])*ch;
            solveCoeff(dm,i);
            sum=sum+(a1*(Math.pow(x_in[i+1],3)-Math.pow(x_in[i],3))/3)+(b1*(Math.pow(x_in[i+1],2)-Math.pow(x_in[i],2))/2)+(c1*(x_in[i+1]-x_in[i]));
            if(i==num_input-1)
            {
                lsum=(a1*(Math.pow(x_in[i+1],3)-Math.pow(x_in[i],3))/3)+(b1*(Math.pow(x_in[i+1],2)-Math.pow(x_in[i],2))/2)+(c1*(x_in[i+1]-x_in[i]));
            }
        }
