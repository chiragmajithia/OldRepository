 ...
 * @author Chirag
 */
public class Interpolation2 extends Applet {
    ...
    public void plot_Trapezoid(Graphics g)
    {
        int i,Xi,Xi1,Yi,Yi1;
        double pixel_dx;
        pixel_dx=(hpix)/interval[TD-1][1];
        g.setColor(Color.red);
        for(i=0;i<num_input;i++)
        {
            Xi = (int) ((x_in[i]* pixel_dx))+offsetX;
            Yi = (int) ((maxpos * Yp_dy) - (y_in[i]* Yp_dy))+offsetY;
            Xi1 = (int) ((x_in[i+1]* pixel_dx))+offsetX;
    ...
    {
        int i,Xp,Yp,Xu,Yu,j;
        double yu,xu=x_in[0],c,mu=0,pixel_dx,xpin,dm,stepx,xp,yp;
        pixel_dx=(hpix)/(interval[TD-1][1]-interval[0][0]);
        g.setColor(Color.BLUE);
        div=10;
        for(i=0;i<num_input-1;i++)
        {
            xpin=x_in[i]+(x_in[i+1]-x_in[i])*0.5;
            dm=(m[i+1]-m[i])/div;
            Xp = (int) (((x_in[i]-interval[0][0])* pixel_dx))+offsetX;
            Yp = (int) ((maxpos * Yp_dy) - (y_in[i]* Yp_dy))+offsetY;
            xp=x_in[i];
            yp=y_in[i];
            for(j=0;j<=div;j++)
            {
    ...
    public void calSlope()
    {
            ...
            for(i=0;i<num_input;i++)
            {
                    m[i]=(y_in[i]-y_in[i+1])/(x_in[i]-x_in[i+1]);
