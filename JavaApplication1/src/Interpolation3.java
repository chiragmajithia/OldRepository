//plot x^3+4x^2-39x-40 from -10 to 10
 ...
 * @author Chirag
 */
public class Interpolation3 extends Applet {

    Point loc = new Point();
    double maxpos, maxneg, llmt, ulmt, Fx, x, nodx, nody, dx, min, Xp_d, Yp_d, Xp_dx, Yp_dy, Xsegwidth, Ysegwidth, NoXseg, NoYseg,ch=0.5,pi=3.1415926535897932384626433832795;
    ...
    double m[]=new double[99];
    int num_input;
    char op[]=new char[10],cher=' ';
    int flagI=0;
    public void paint(Graphics g) {
       ...
       addMouseListener(new MouseAdapter() {
            ...
            /*public void mouseClicked(MouseEvent e)
            {
               ...
               Point x = e.getPoint();
               print = interval[0][0] + ((x.x - offsetX) / Xp_dx * dx);
                if(flagI!=0)
                {
                        if(cher=='q')
                        {
                        x_in[flagI-1]=print;
                        y_in[flagI-1]=readFx(print);
                        System.out.println("Entered value of X["+(flagI-1)+"] = "+print+"Y= "+y_in[flagI-1]);
                        flagI++;
                        cher=' ';
                        }
                        if(flagI-1==num_input)
                        flagI=0;
                }
    ...
    {
        int i,Xp,Yp,Xu,Yu,j;
        double yu,xu=x_in[0],c,mu,pixel_dx,xpin,dm,stepx,xp,yp;
        pixel_dx=(hpix)/(interval[TD-1][1]-interval[0][0]);
        Xp=0;
        Yp=0;
        ...
        {
            xpin=x_in[i]+(x_in[i+1]-x_in[i])*ch;
            dm=m[i+1]-m[i];
            if(dm>pi)
            {
                dm=dm-pi;
                //System.out.println("dm=dm-pi"+((dm-pi)*180/pi)+"-"+(pi*180/pi)+" = "+(dm*180/pi));
            ...
            dm/=div;
            xu=x_in[i];
            //System.out.println("mi in deg"+(m[i]*180/pi)+"mi+1 in deg"+(m[i+1]*180/pi)+"difference ="+((m[i+1]-m[i])*180/pi)+"xu= "+xu+"dm"+(dm*180/pi));
            for(j=0;j<=div;j++)
            {
                if(xu>x_in[i+1])
                {
                ...
                mu=Math.tan(m[i]+dm*j);
                //System.out.println("m["+i+"]+ dx * j ="+(m[i]*180/pi)+" + "+ (dm*180/pi)+" * "+j+" = "+(Math.atan(mu)*180/pi));
                yu=(mu*xu)+y_in[i+1]-(mu*x_in[i+1]);
                Xu = (int) (((xu-interval[0][0])* pixel_dx))+offsetX;
                Yu = (int) ((maxpos * Yp_dy) - (yu* Yp_dy))+offsetY;
                System.out.println("xu= "+xu+"yu= "+yu+"j= "+j);
  ...
  /*          try
    ...
    public void calSlope()
    {
            ...
            for(i=0;i<num_input;i++)
            {
                    m[i]=Math.atan2((y_in[i+1]-y_in[i]),(x_in[i+1]-x_in[i]));
                    System.out.println("i= "+i+" y2-y1 "+(y_in[i+1]-y_in[i])+" x2-x1 "+(x_in[i+1]-x_in[i])+" Slope "+i+" ="+(m[i]*180/3.1415926535897932384626433832795));
            }
    }
      ...
      /*  while(i!=-999)
        {
            ...
            {}
            System.out.println("Fx= "+readFx(i));
       }
       */
    }
