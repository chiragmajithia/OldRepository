 ...
 * @author Chirag
 */
public class Interpolation5 extends Applet {

    Point loc = new Point();
    double maxpos, maxneg, llmt, ulmt, Fx, x, nodx, nody, dx, min, Xp_d, Yp_d, Xp_dx, Yp_dy, Xsegwidth, Ysegwidth, NoXseg, NoYseg,ch=0.6;
   ...
   addKeyListener(new KeyAdapter()
    ...
    public void plot_Interpolation(Graphics g)
    {
        ...
        for(i=0;i<num_input-1;i++)
        {
            ...
            if(Math.abs(dd)<=90)
            {
                ch=0.85+(0.15/pi*2*Math.abs(dd));
            }
            if(Math.abs(dd)>=90)
            {
                ch=0.98-(0.20/pi*Math.abs(dd));
            }
