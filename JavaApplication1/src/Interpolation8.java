 ...
 * @author Chirag
 */
public class Interpolation8 extends Applet {
   ...
   addKeyListener(new KeyAdapter()
    ...
    {
        int i,Xp,Yp,Xu,Yu,j;
        double yu,xu=x_in[0],c,mu,pixel_dx,xpin,dm,stepx,xp,yp,diff;
        pixel_dx=(hpix)/(interval[TD-1][1]-interval[0][0]);
        Xp=0;
        Yp=0;
        ...
        for(i=0;i<num_input-1;i++)
        {
            ...
            ch=0.1;
            min= Math.abs(differentiation(x_in[i+1])-(m[i]+(m[i+1]-m[i])*ch));
            while(true)
            {
                ch+=0.01;
                diff=Math.abs(differentiation(x_in[i+1])-(m[i]+(m[i+1]-m[i])*ch));
                if(diff<min)
                {
                    min=diff;
                }
   ...
   public double Interpolation(double xc)
    ...
    public double differentiation(double xc)
    {
           return ((readFx(xc+0.001)-readFx(xc-0.001))/0.002);
    }
