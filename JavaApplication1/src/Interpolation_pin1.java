 ...
 * @author Chirag
 */
public class Interpolation_pin1 extends Applet {

    Point loc = new Point();
    double maxpos, maxneg, llmt, ulmt, Fx, x, nodx, nody, dx, min, Xp_d, Yp_d, Xp_dx, Yp_dy, Xsegwidth, Ysegwidth, NoXseg, NoYseg,ch=0.5,pn=0.5;
   ...
   addKeyListener(new KeyAdapter()
            ...
            public void keyPressed(KeyEvent e)
            {
                ...
                if(e.getKeyChar()=='c')
                {
                    try
                    {
                        ...
                        System.out.println("Set the coefficient 0<p<1");
                        s=inp.readLine();
                        pn=Double.parseDouble(s);
                        table();
                    }
                    catch(IOException E)
                    {}
    ...
    public void plot_Interpolation(Graphics g)
    {
        ...
        {
            xpin=x_in[i]+(x_in[i+1]-x_in[i])*pn;
            dm=(m[i+1]-m[i])*ch/div;
            xu=x_in[i];
