...
public void init()
{
      ...
      String s;
        ...
        do
        {
            System.out.print("Enter any key to change other variables... 'n' to terminate");
            c=(char)inp.read();
            if(c=='n')
                break;
            else
            {
            initVariables();
            System.out.println("Integral of function from "+interval[0][0]+" to "+interval[TD-1][0]+" is:: "+trapezoid());
            fourierCoeff();
            }
        }
        while(c!='n');
...
void calCoeff() throws IOException
{
    InputStreamReader r=new InputStreamReader(System.in);
    BufferedReader inp=new BufferedReader(r);
    String s;
...
void fourierCoeff()
{
    ...
    System.out.print("Sr no         n       An          Bn      Cn");
    for(i=f;i<=l;i++)
    {
